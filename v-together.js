...
put(T("GitHub Actions + ESLint + Jest + CodeQL + Trivy","One required check set on the PR. Language linters swap (Ruff, golangci-lint) but the shape stays.",[gha,esl,jest,cql,trivy]),["ci","cicd","ci-pipelines","gate-lint","gate-static","gate-type","gate-compile","gate-unit","gate-security","gate-deps","step3","step4"]);
put(T("Testcontainers + Pact + Playwright + Kind","Real Postgres Redis Kafka in CI, contract against app-users, browser on GET /feed, cluster when the API server is the fixture. LocalStack only if the suite needs AWS APIs.",[tc,pact,pw,kind]),["suites","step5","coordinator","workers","results"]);
put(T("GitHub + Argo CD + Kubernetes","PR merges, Argo syncs the Deployment. CI does not talk to the cluster.",[gh,argo,k8s]),["git","argocd","platform-gitops","repo-manifests","bump","repo-app","app-newsfeed","app-users","pr-4821","step2","step8","step9"]);
put(T("Kubernetes + Kyverno + OPA","Core plus admission plugins. Kyverno is the usual YAML path. OPA when the policy is Rego.",[k8s,kyv,opa]),["admission","gate-policy","step10"]);
...
put(T("OpenTelemetry + Prometheus + Grafana","One SDK, scrape or remote-write, one dashboard. Tempo or Loki join when you want traces or logs in the same Grafana.",[otel,prom,gra,tp,lk]),["otel-sdk","otel-collector","step12"]);
...
put(T("Docker Build + Harbor or ECR","Image is the artifact. Registry is where Argo pulls from.",[db,hbr,ecr]),["registry","image-v1827","build","step6","step7","step11"]);
...
if(D.suites&&D.suites.d)D.suites.d.push("Usual combo is Testcontainers + Pact + Playwright + Kind.");
})(window.IDP_DATA);
