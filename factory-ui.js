...
function render(id) {
  ...
  let extra = "";
  const oh = officialHref(item.seeHref);
  const lh = safeHref(item.seeHref);
  if (oh) {
    extra += "<h3>Official</h3><p><a href=\"" + oh + "\" target=\"_blank\" rel=\"noopener\">" +
      escapeHtml(item.seeLabel || item.n) + "</a></p>";
  } else if (lh) {
    const t = String(item.seeTab || "tab");
    const pretty = t === "v2" ? "v2" : t.charAt(0).toUpperCase() + t.slice(1);
    extra += "<h3>Open in " + escapeHtml(pretty) + "</h3><p><a href=\"" + lh + "\">" +
...
Promise.all(Array.from({length: SVG_N}, function (_, i) {
  return fetch("factory-svg-" + i + ".txt").then(function (r) { return r.text(); });
})).then(function (parts) {
