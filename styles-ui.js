...
function renderExtra(item) {
  let extra = "";
  if (item.example) extra += "<h3>Used as</h3><p>" + escapeHtml(item.example) + "</p>";
  if (item.whyPick) extra += "<h3>Why it's used</h3><p>" + escapeHtml(item.whyPick) + "</p>";
  if (item.scale) extra += "<h3>Common scale</h3><p>" + escapeHtml(item.scale) + "</p>";
  ...
  if (item.plane && item.plane.length) {
    extra += "<h3>On Plane</h3><p>" + item.plane.map(function (x) {
      const h = safeHref(x.href);
      return h ? "<a href=\"" + h + "\">" + escapeHtml(x.n) + "</a>" : escapeHtml(x.n);
    }).join(" · ") + "</p>";
  }
...
Promise.all(Array.from({length: SVG_N}, function (_, i) {
  return fetch("y-svg-" + i + ".txt").then(function (r) { return r.text(); });
})).then(function (parts) {
