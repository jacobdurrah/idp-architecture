...
- 🟡 **IG-05 — Integration + e2e + deploy (join).** 🤖👤 *(Integration + deploy DONE 2026-08-23; final e2e BLOCKED on Vercel AI Gateway billing — the agent loads and enters its tool loop but the gateway returns 403 `customer_verification_required` until a card is added to the jacob-durrahs-projects team. Dock live on all 6 tabs pointing at https://idp-guide.vercel.app.)* Merge A+B+C in `idp-architecture` (one PR: "Add guide dock, hash-open, and catalog.json"). Deploy `idp-guide` to Vercel; flip the dock's default origin to the production URL. Deploy the dock + hash + catalog to Pages (or a PR for Jacob to merge). **Impact (plain terms):** proves the whole thing works end to end on the real site — ask a question on one page, land on the right box on another.
