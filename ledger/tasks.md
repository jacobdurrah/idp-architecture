...
- ✅ **IG-14 — Test harness + runner + Actions button (idp-guide).** 🔨 *(DONE 2026-08-23 — `npm test` = tsc + node:test lookup evals, 8/8 green; `tests.yml` with Run workflow button; README section.)* Same for the eve repo: `npm test` runs the agent eval + embed tests, a `workflow_dispatch` "Run workflow" button, README section. **Impact (plain terms):** the same one-command / one-button check for the guide app.
...
- ✅ **IG-05 — Integration + e2e + deploy (join).** 🤖👤 *(DONE 2026-08-25 — v1 works end to end. Agent switched to OpenAI `gpt-4o` (direct, `@ai-sdk/openai`) after the AI Gateway demanded a card; `OPENAI_API_KEY` set on Vercel. Trace-verified: "why is transatlantic 65-75 ms?" → `lookup` then `open_box` fire, answer grounded in real catalog copy (4.9 μs/km), cites a Metal box. Dock live on all 6 tabs → https://idp-guide.vercel.app; embed public, HTTP 200, `frame-ancestors` CSP allows Pages. Catalog live + auto-regenerating on Pages.)* Merge A+B+C in `idp-architecture` (one PR: "Add guide dock, hash-open, and catalog.json"). Deploy `idp-guide` to Vercel; flip the dock's default origin to the production URL. Deploy the dock + hash + catalog to Pages (or a PR for Jacob to merge). **Impact (plain terms):** proves the whole thing works end to end on the real site — ask a question on one page, land on the right box on another.
...
## Log
- 2026-08-25 — **v1 COMPLETE end to end.** No usable Anthropic key on the device (empty token; only an OAuth token), so the model is **OpenAI `gpt-4o` direct** (`@ai-sdk/openai`, `OPENAI_API_KEY` set on Vercel prod+dev) — bypasses the AI Gateway card gate. Redeployed. Trace-verified the canonical flow: `lookup` → `open_box` both fire, answer grounded (4.9 μs/km, cites a real Metal box). Every link live: catalog on Pages (491 records, auto-regen) → agent grounded → public embed (CSP allows Pages) → dock on all 6 tabs. **Remaining polish (non-blocking):** tune the prompt to prefer the most specific hop (it cited `backbone` vs `subsea` on the bare question) and to always call `open_box`; add dock + hash-load to `scenarios.html`/`scenarios-ui.js` once that page settles; IG-12 consumer-side (idp-guide) schema validation; v2/v3. **Standing actions for Jacob: (1) rotate the `VERCEL_TOKEN`** (CLI echoed it once). **(2)** OpenAI usage now bills to the `OPENAI_API_KEY` account; the embed is public (`none()` auth) with per-session token caps — tighten if needed.
- 2026-08-23 — **Stream D built + deployed.** `jacobdurrah/idp-guide` (private) created; eve app (Node 25) with `lookup`+`open_box`, grounded prompt, embed postMessage bridge, public auth, per-session token caps; `npm test` green (tsc + 8 lookup evals). Linked to Vercel team `jacob-durrahs-projects`, deployed to **https://idp-guide.vercel.app** (public, HTTP 200, correct `frame-ancestors` CSP, no SSO wall). Dock re-enabled + repointed to the app root (another session had disabled it assuming no deploy). Catalog live on Pages, auto-regenerated to 491 records by IG-08 as new shards landed. **BLOCKERS/ACTIONS: (1)** the agent cannot answer until a **credit card is added to Vercel AI Gateway** (403 `customer_verification_required`) — or switch the model to a direct Anthropic/OpenAI key from `~/.hermes/.env`. **(2)** the `VERCEL_TOKEN` was echoed once by the Vercel CLI into this session's output — **rotate it** at vercel.com/account/tokens.
...
- 2026-08-20 — **v1 in-repo streams DONE and integrated — `npm test` green (11 unit/contract + 14 e2e).** All 6 subagents landed; merged cleanly (disjoint files). Integration caught two real defects test-first: the contract test needed ajv's draft-2020 build, and `guide-dock.js` tab detection broke under clean URLs (vercel.json `cleanUrls`) — both fixed. **Living-site pass:** the 6th tab **Scenarios** landed on main mid-build; absorbed through the data seam with a one-line generator map (`s→scenarios`) + schema enum + test map — catalog grew 389→415 records, no rewrite. **Deferred:** dock + hash-load on `scenarios.html`/`scenarios-ui.js` (that file is still being actively built by another session; will add once it settles). Stream D (eve app) still needs Jacob's Vercel + GitHub.
