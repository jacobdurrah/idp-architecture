...
function render(id) {
  const item = DATA[id] || DATA["inv-overview"];
  current = DATA[id] ? id : "inv-overview";
  ...
  let extra = "";
  if (item.payload) extra += "<h3>Payload (EXAMPLE)</h3><pre>" + escapeHtml(item.payload) + "</pre>";
...
function highlight(id) {
  ...
  if (!hits.length || id === "inv-overview") { selbox.setAttribute("visibility", "hidden"); return; }
...
function bind() {
  ...
  document.getElementById("poster").addEventListener("click", function (evt) {
    ...
    if (!mq.matches) render("inv-overview");
  });
  const h = (location.hash || "").replace(/^#/, "");
  if (findStory(h)) playStory(h);
  else if (h && DATA[h]) { const s = storyIdFor(h); if (s) setStory(s); render(h); }
  else render("inv-overview");
  if (mq.matches && current === "inv-overview") panel.classList.remove("is-open");
}
const SVG_N = 2;
Promise.all(Array.from({length: SVG_N}, function (_, i) {
  return fetch("inventory-svg-" + i + ".txt").then(function (r) { return r.text(); });
})).then(function (parts) {
