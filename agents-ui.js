...
let activeStory = (SCENARIOS[0] && SCENARIOS[0].id) || "ship";
...
g.setAttribute("opacity", g.getAttribute("data-band") === activeStory ? "1" : "0.38");
});
}
...
const band = document.querySelector('[data-band="' + id + '"]');
if (band && band.scrollIntoView) band.scrollIntoView({ block: "nearest", behavior: "smooth" });
}
function stepBy(dir) {
const s = findStory(activeStory);
